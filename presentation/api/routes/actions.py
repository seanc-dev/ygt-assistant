...
from infra.repos.factory import audit_repo  # archived; replace with in-memory if needed
...
async def actions_scan(body: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        rid = "scan"
        audit_repo().write({"request_id": rid, "tenant_id": TENANT_DEFAULT, "dry_run": True, "actions": approvals, "results": {"source": "actions_scan"}})
    except Exception:
        pass
    return approvals
...
async def actions_undo(approval_id: str) -> Dict[str, Any]:
    ...
    if not a:
        return {"error": "not_found"}
    a["status"] = "proposed"
    history_log.append({"ts": "now", "verb": "undo", "object": "approval", "id": approval_id})
    return a
