...
create type task_status as enum ('backlog', 'ready', 'doing', 'done', 'blocked');
...
create index if not exists schedule_blocks_user_day_idx
  on schedule_blocks (tenant_id, user_id, day);
...
create index if not exists action_items_queue_idx
...
create unique index if not exists docs_drive_item_idx
  on docs (tenant_id, drive_id, item_id);
...
create index if not exists doc_events_doc_idx
  on doc_events (doc_id, happened_at);
...
create index if not exists core_memory_tenant_level_idx
  on core_memory_items (tenant_id, level);

create index if not exists core_memory_embedding_idx
  on core_memory_items using ivfflat (embedding vector_cosine_ops);
