...
create policy tenants_select_self
  on tenants
  for select
  using (auth_helpers.is_service_role() or id = auth_helpers.current_tenant_id());

create policy tenants_mutate_service
  on tenants
  for all
  using (auth_helpers.is_service_role())
  with check (auth_helpers.is_service_role());
