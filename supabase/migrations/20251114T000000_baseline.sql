-- Baseline schema for LucidWork Supabase/Postgres deployment
...
create type task_status as enum ('backlog', 'doing', 'done', 'blocked');
...
create table if not exists users (
  ...
  workday_start time default '09:00'::time,
  workday_end time default '17:00'::time,
...
create table if not exists connections_ms (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  tenant_id uuid not null references tenants(id) on delete cascade,
...
create index if not exists tasks_tenant_project_idx on tasks (tenant_id, project_id);

-- 4) THREADS + MESSAGES (INLINE CHAT + WORKROOM)
...
create index if not exists threads_tenant_task_idx on threads (tenant_id, task_id);
...
-- 5) SCHEDULE BLOCKS (TODAY-FIRST + ALTERNATIVES)
...
create index if not exists schedule_blocks_user_day_idx on schedule_blocks (tenant_id, user_id, day);

-- 6) ACTION QUEUE / DEFER / SCHEDULE LINK
...
create unique index if not exists action_items_dedupe_idx on action_items (
    tenant_id,
    owner_id,
    source_type,
    dedupe_key
...
create index if not exists action_items_queue_idx on action_items (
    tenant_id,
    owner_id,
    state,
    priority,
...
create unique index if not exists docs_drive_item_idx on docs (tenant_id, drive_id, item_id);
...
create index if not exists doc_events_doc_idx on doc_events (doc_id, happened_at);
...
create index if not exists core_memory_tenant_level_idx on core_memory_items (tenant_id, level);

create index if not exists core_memory_embedding_idx on core_memory_items using ivfflat (embedding vector_cosine_ops);
...
create index if not exists audit_tenant_ts_idx on audit_log (tenant_id, created_at);
