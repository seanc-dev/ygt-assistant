...
class MicrosoftEmailProvider(EmailProvider):
    ...
    def list_threads(self, q: str, max_n: int) -> List[Dict[str, Any]]:
        ...
        async def _run() -> List[Dict[str, Any]]:
            ...
            async with httpx.AsyncClient(timeout=10) as c:
                r = await c.get(f"{self._base()}/me/messages", params=params, headers={"Authorization": f"Bearer {token}"})
                if r.status_code == 401:
                    raise ProviderError("microsoft", "list_threads", "unauthorized", status_code=401, hint="Reconnect Microsoft")
                r.raise_for_status()
                data = r.json()
                ...
                return [
                    {
                        ...
                        "to": [x.get("emailAddress", {}).get("address") for x in (it.get("toRecipients") or [])],
    ...
    def create_draft(self, to: List[str], subject: str, body: str) -> Dict[str, Any]:
        ...
        async def _run() -> Dict[str, Any]:
            ...
            async with httpx.AsyncClient(timeout=10) as c:
                r = await c.post(f"{self._base()}/me/messages", json=msg, headers={"Authorization": f"Bearer {token}"})
                r.raise_for_status()
                data = r.json()
                return {"id": data.get("id"), "to": to, "subject": subject, "body": body, "status": "draft"}
    ...
    def send_draft(self, draft_id: str) -> Dict[str, Any]:
        ...
        async def _run() -> Dict[str, Any]:
            token = await self._auth()
            async with httpx.AsyncClient(timeout=10) as c:
                r = await c.post(f"{self._base()}/me/messages/{draft_id}/send", headers={"Authorization": f"Bearer {token}"})
    ...
    def send_message(self, to: List[str], subject: str, body: str) -> Dict[str, Any]:
        ...
        async def _run() -> Dict[str, Any]:
            ...
            async with httpx.AsyncClient(timeout=10) as c:
                r = await c.post(f"{self._base()}/me/sendMail", json=payload, headers={"Authorization": f"Bearer {token}"})
                if r.status_code not in (202, 200, 204):
